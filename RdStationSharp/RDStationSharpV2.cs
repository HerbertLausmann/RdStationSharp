...
{
    public class RDStationSharpV2 : HL.MVVM.ViewModelBase
    {
        private Data.LocalDataSource _Source;
        private RDs.RdStationClient _Client;

        public LocalDataSource Source { get => _Source; private set => _Source = value; }
           ...
           GetCommand(new HL.MVVM.Threading.ThreadRelayCommand(
                   ...
                   HL.MVVM.Threading.ThreadRelayCommand.ThreadRelayCommandExecuteEventArgs e) =>
               {
                   ...
                   try
                   {
                       ...
                       RDCloud.Default.Initialize(_Client, e);
                   }
                   catch
                   {
                       Helper.RDUserLog.Default.Write("Houve uma falha na conexão com a RD Cloud. Encerrando atividade");
                       Helper.RDUserLog.Default.Finish();
                       return;
                   }

                   e.ReportProgressText("Total de Leads na RD Station: " + RDCloud.Default.Leads.Count.ToString());
                   Helper.RDUserLog.Default.Write("Total de Leads na RD Station: " + RDCloud.Default.Leads.Count.ToString());

                   try
                   {
                       e.ReportProgressText("Iniciando limpeza da RD Station!");
                       Helper.RDUserLog.Default.Write("Iniciando limpeza da RD Station!");
                       RDCloud.Default.CleanCloud();
                       e.ReportProgressText("Total de Leads na RD Station após limpeza: " + RDCloud.Default.Leads.Count.ToString());
                       Helper.RDUserLog.Default.Write("Total de Leads após limpeza: " + RDCloud.Default.Leads.Count.ToString());
                   }
                   catch
                   {
                       Helper.RDUserLog.Default.Write("A limpeza da base da RD Station falhou!");
                   }
                   ...
                   for (int i = 0; i < Source.Table.Rows.Count; i++)
                   {
                       try
                       {
                           ...
                           {
                               e.ReportProgressText(string.Format("Lead {0} é está marcado como OptOut.", currentRow["E-mail"].ToString()));
                               continue;
                           }
        ...
        {
            currentTask.ReportProgressText(">>> Getting data from RD Clound: " + lead.Payload.Email);
            var leadRD = RDCloud.Default.GetLeadByEmail(lead.Payload.Email);
